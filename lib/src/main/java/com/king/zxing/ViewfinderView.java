...
public final class ViewfinderView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        frame = new Rect(leftOffset-150, topOffset, leftOffset + frameWidth+150, topOffset + frameHeight);
    }
...
//        canvas.drawRect(frame.right - cornerRectHeight, frame.bottom - cornerRectWidth, frame.right, frame.bottom, paint);
    ...
    private void drawLineScanner(Canvas canvas,Rect frame){
        ...
        if(scannerStart <= scannerEnd) {
            //椭圆 left top right bottom
            /**
             * Create a new rectangle with the specified coordinates. Note: no range
             * checking is performed, so the caller must ensure that left <= right and
             *  top <= bottom.
             */
            Log.i("viewfinderview","left = "+(frame.left + 2 * scannerLineHeight));
            Log.i("viewfinderview","top = "+scannerStart);
            Log.i("viewfinderview","right = "+ (frame.right - 2 * scannerLineHeight));
            Log.i("viewfinderview","bottom = "+(scannerStart + scannerLineHeight));
            Rect rectF = new Rect(frame.left + 2 * scannerLineHeight, scannerStart, frame.right - 2 * scannerLineHeight, scannerStart + scannerLineHeight+20);
//            canvas.reac(rectF, paint);
...
//        return mContext.getResources().getColor(R.color.viewfinder_laser);
